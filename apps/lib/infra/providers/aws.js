...
module.exports.AwsProvider = {
  create: async ({
      ...
      objectStorage: {
        uploadFile: async (bucket, key, data) => {
          ...
          const { Location } = await s3
            .upload({ Bucket: bucket, Key: key, Body: data })
            .promise()
      ...
      faas: {
        ...
        createHttpFunction: async (
          ...
          return {
            ...
            }.execute-api.${region}.amazonaws.com/${faasRestGatewayStage}/${name}`,
          }
        },
    ...
    const createApiGateway = async () => {
      ...
      const { id: restApiId } = await apigtw
        .createRestApi({
          name: projectName,
    ...
    const addLambdaToApiGateway = async ({ name, lambdaArn }) => {
      ...
      const { id: apiGtwResId, path: apiGtwResPath } = await limiter.schedule(
        () =>
          apigtw
            .createResource({
              ...faasRestGateway,
